...
// 테스트 케이스 개수가 주어짐
// 각 테스트 케이스에 해당하는 학생 수 와 학생우에 대한 성적 입력
...
public class OVerScoreMain {
    public static void main(String[] args) {
        ...
        for(int i = 0; i < testCnt; i++) { // 테스트케이스 횟수만큼 반복해서 overRate() 호출
            result[i] = overRate();
        }
        // 반환된 결과값을 출력
        for(double e : result) {
    ...
    static double overRate() {
        ...
        int total = 0; // 평균을 구하기 위해서 학생의 성적을 합산 함.
        int overCnt = 0; // 평균을 넘어선 학생 수
        int stCnt = sc.nextInt();
        int[] score = new int[stCnt]; // 학생의 수 만큼 성적을 입력 받기 위한 배열 생성
        // 반복문을 순회하면서 학생의 수만큼 성적 입력 받기
        for(int i = 0; i < stCnt; i++) {
            score[i] = sc.nextInt();
            total += score[i]; // 학생의 성적을 누적 함.
        }
