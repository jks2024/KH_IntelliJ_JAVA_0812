...
public class CheckPWClass {
    String password;
    boolean inputPW(String password) {
        // 패스워드 체크를 위해 전달받은 매개변수를 인스턴스 필드에 복사
        // 인스턴스 password를 사용하지 않는다면 해당 메소드에서 기능 전부를 구현해야 함.
...
//        System.out.println("특수문자체크 : " + validRex());
        ...
        // 모든 체크 조건을 다 만족해야 "Good password!" 조건이 성립 함.
    ...
    boolean validLength() {
        // 메소드의 반환타입은 해당 메소드가 호출된 후 호출한 곳으로 수행 이후 결과를 반환 함
    ...
    boolean validNumber() {
        for(int i = 0; i < password.length(); i++) {
            // charAt(인덱스)는 문자열 해당 인덱스의 문자를 반환함.
            // (문자는 정수로 계산되기때문에 범위 체크 가능)
            // 해당하는 조건문내로 한번이라도 진입하게되면 해당 메소드는 true 반환 되고 끝남
            if(password.charAt(i) >= '0' && password.charAt(i) <= '9') {
                return true; // return 문은 해당 메소드를 끝내고 호출한 곳으로 이동 함
                // 리턴문에 오는 값은 반드시 반환 타입과 같아야 함.
            }
        }
        // for 문내에 있는 리턴문을 만나지 못한 경우 해당 리턴문이 수행 된다.
        return false;
    }
    ...
    boolean validRex() {
        ...
        // 2중 for : 특수문자의 길이만큼, 각 인덱스의 문자를 입력 받은 문자열 전체를 비교
