...
// push(item) : 객체를 저장, 맨위에서 동작이 일어남
// peek() : 스택 맨 위의 값 확인
// pop() : 스택 맨 위의 값을 추출하면서 반환 해 준다.
// empty() : 스택이 비어 있는지 확인 한다.
// [사용용도]
// 프로그램의 Call Stack에 사용 (메소드 호출 관계)
// 인터넷에서 경로 이동 시 이전 경로에 대한 이동 관린 등에 사용
// 문서 작성 시 Undo 기능 등에 사용 됨
// 문서 편집기 등에서 괄호 열림 닫힘 판별하기 등에서 사용
public class Stack1 {
    public static void main(String[] args) {
        Stack<Integer> stack = new Stack<>(); // 스택 객체 생성
        stack.push(1); // 객체를 저장, 맨위에서 동작 일어 남
        stack.push(2);
        stack.push(3);
        // Stack의 맨 위의 값 확인
        System.out.println(stack.peek());
        // Stack의 맨 위의 값 추출 및 보여 줌
        System.out.println(stack.pop());
        // 스택에 포함된 객체 보기
        System.out.println(stack);
        // 스택이 비어 있는지 확인
        System.out.println(stack.empty());
        // 스택의 사이즈 확인 (값이 몇개 들어 있는지??)
