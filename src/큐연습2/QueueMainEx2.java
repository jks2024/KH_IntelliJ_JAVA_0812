...
public class QueueMainEx2 {
    ...
    public static void main(String[] args) {
        ...
        while(true) { // "q/Q" 문자가 들어 올 때까지 계속 반복 수행
            System.out.print(">> "); // 터미널레서 입력 화면처럼 보여 주기 위해서 만듬
            Scanner sc = new Scanner(System.in);
            String in = sc.nextLine().trim(); // 입력 받은 문자열에의 앞/뒤의 공백을 지워 줌
            if(in.equalsIgnoreCase("q")) {
                System.exit(0); // 강제 종료 시킴, System Call
                // equalsIgnoreCase() 문자열 비교 시 재소문자 구분 하지 않음
            } else if(in.equalsIgnoreCase("help")) {
                System.out.println("help - 도움말을 보여 줍니다.");
                System.out.println("q/Q - 프로그램을 종료 합니다.");
                System.out.println("history - 최근 입력한 명령어를 "
                        + MAX_SIZE + "개 보여 줍니다.");
            } else if(in.equalsIgnoreCase("history")) {
                save(in); // 입력 받은 문자열을 queue에 저장하는 메소드
                int cnt = 0;
                for(String e : queue) {
                    cnt++; // 명령 history앞에 숫자를 붙여 주기 위해서 만든 변수
